...
class BKFilterView: UIView {
    ...
    internal func revealCircular(duration: CFTimeInterval = 0.3, completion: (() -> Void)?)
    {
        ...
        let maskLayerBounds: CGRect = CGRect(x: 0, y: 0, width: maskLayerEdgeLength, height: maskLayerEdgeLength)
        
        let maskLayerPosition: CGPoint = CGPoint(x: self.bounds.size.width/2.0, y: self.bounds.size.height/2.0)
        
        let maskLayer = CAShapeLayer()
        
        maskLayer.bounds = CGRect(x: (self.bounds.size.width-1.0)/2.0, y: (self.bounds.size.height-1.0)/2.0, width: 1, height: 1)
    ...
    internal func hideCircular(duration: CFTimeInterval = 0.3, completion: (() -> Void)?)
    {
        ...
        maskLayer.bounds = CGRect(x: 0, y: 0, width: (2 * fromRadius), height: (2 * fromRadius))
